...
- **Streaming auto-scroll**: never use `scrollIntoView({ behavior: "smooth" })` on every `messages` change — during streaming, `messages` mutates per-token, which yanks the user to the bottom on every token and makes it impossible to scroll up and read prior output. Use the "stick to bottom" pattern: track whether the user is within ~80px of the bottom via a scroll listener + ref, and only auto-scroll if pinned. Use `behavior: "auto"` (instant) for streaming chunks, `"smooth"` only for discrete events like new message added. Skill: `.cursor/skills/frontend/streaming-autoscroll.md`.
